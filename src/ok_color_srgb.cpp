...
ok_srgb_888 ok_srgb_888_from(ok_lrgb_888 lin) {
  (void) lin;
  return {};
}
...
// "Electro-Optical Transfer Function" (EOTF)
// sRGB 8-bit to linear 8-bit lookup table (256B), rounded from s8_l16 above
static const uint16_t s8_l8[] = {
...
ok_lrgb_888 ok_lrgb_888_from(ok_srgb_888 std) {
  return { s8_l8[std.r], s8_l8[std.g], s8_l8[std.b] };
}

ok_lrgb_HHH ok_lrgb_HHH_from(ok_srgb_888 std) {
  return { s8_l16[std.r], s8_l16[std.g], s8_l16[std.b] };
}

static uint16_t s16_l16(uint16_t s) {
  // Linear interpolation based on s8_l16
  uint8_t const s_hi = s / 0x100, s_lo = s & 0x100;
  auto const s_a = s8_l16[s_hi];
  auto const s_b = s8_l16[s_hi + 1];
  auto const s_diff = s_b - s_a;
  auto const s_frac = s_lo * s_diff / 0x100;
